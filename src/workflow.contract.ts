...
// Connection transports and the capability catalogue live in their own contracts now; the barrel
// re-exports both, because in this package a module with exports of its own may not carry an
// `export *` (it makes a self-referential cycle that ESM interop can resolve to `undefined`).
import { WorkflowMcpConnectionSettings } from "./workflow.connections.contract";
import { WORKFLOW_CAPABILITIES, WORKFLOW_IMPLEMENTED_CAPABILITIES, WorkflowCapabilityBroker, findWorkflowCapability } from "./workflow.capabilities.contract";
