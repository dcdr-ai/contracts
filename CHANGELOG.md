...
<!--
sourceCommit: 9a9e4561be9c8bc8486a256f3767f389b016362d
queuedAtUtc: 
previousMirroredBuild: 20260914.1 (2026-09-14)
contractsSubmodule: 8e78bde901a1..3a7e77d10d1e
-->

### Added
- <kbd>RUNTIME</kbd> **Claude Code's capability headers reach Anthropic through the gateway.** `/v1/messages` forwards `anthropic-*` request headers as an open list, with room for a full `anthropic-beta` capability list; a long list was being dropped, which turns Claude Code features that pair a beta header with a body field into provider `400`s.
- <kbd>CONTRACTS</kbd> **`docs/GATEWAY.md`: the AI Gateway user guide** - authentication, endpoints, models, streaming, limits, errors and tool setup.
- <kbd>RUNTIME</kbd> **Service tokens can be restricted to a list of models.** A token carrying `allowedModels` only reaches those provider/model pairs: `/v1/models` lists only them, the gateway refuses other models with `model_not_allowed_for_token`, and intent execution skips candidates outside the list, failing with `MODEL_NOT_ALLOWED_FOR_TOKEN` when none is left. Both are `403` and distinct from a model disabled for the whole workspace.
- <kbd>RUNTIME</kbd> **Service-token limit rules, on intent execution and on the gateway.** A token can carry several limits at once - calls or budget, per hour, day, ISO week or month, scoped to surfaces, intents, providers or models - and every rule that matches a request must have room. A rule scoped to providers or models counts each upstream call (retries and fallbacks included, cache hits not); any other rule counts requests. A request refused by one rule gives back what the others took for it. Gateway tokens are limited for the first time. Refusals answer `429` with `Retry-After` and the new code `SERVICE_TOKEN_LIMIT_EXCEEDED` (gateway: `service_token_limit_exceeded`). Tokens that still carry the older `limits` keep exactly their current behaviour and counters.
- <kbd>RUNTIME</kbd> **Gateway logs say whether a call reached the provider.** `gateway.dispatched` is `true` once a request passed every admission gate and the upstream call was attempted - also when the provider answered with an error or the client disconnected - and `false` for every refusal before it, so a control plane can count calls without interpreting error codes. Provider Limits refusals on the gateway now carry `Retry-After`.
- <kbd>RUNTIME</kbd> **The runtime tells the control plane what it enforces.** Every call to the control plane carries `x-dcdr-capabilities`, and `/api/system/version` lists the same capabilities, so an editor can mark a rule as not enforced until every runtime of the tenant supports it.
- <kbd>RUNTIME</kbd> **Token usage means the same thing on every provider and surface.** `promptTokens` now includes cached reads and cache writes everywhere (Anthropic reported them apart), and logs carry `cachedPromptTokens`, `cacheWritePromptTokens` and `cacheWrite1hPromptTokens` where the provider reports them, so cache-heavy traffic can be priced correctly.
- <kbd>CONTRACTS</kbd> **`@dcdr/contracts` 3.14.0 - token limit rules, dispatched gateway calls, cache pricing.** `DcdrServiceTokenLimitRule` (with metric, window, surface and kind enums, ISO-week period keys and the legacy translation), `rules` / `name` / `kind` on the snapshot item and the gateway token check, `ExecutionErrorCode.SERVICE_TOKEN_LIMIT_EXCEEDED`, `GatewayExecutionLogDetails.dispatched`, cache-write usage counters, `cacheWriteInput` / `cacheWrite1hInput` on token pricing (every Anthropic model priced), and runtime capability keys with `DCDR_RUNTIME_CAPABILITIES_HEADER`. Additive; `limits` is deprecated in favour of `rules`.
- <kbd>RUNTIME</kbd> **The `/v1` gateway streams, and relays the provider as it is.** `stream: true` on `/v1/chat/completions` and `/v1/responses` now relays the upstream event stream event by event, so coding agents and chat tools that need tokens as they come can point at the gateway. The dispatch no longer goes through the OpenAI SDK: the client's body is forwarded as sent, and the provider's status, headers and body - error bodies included - come back untouched, so a tool reads the vendor's own rate-limit or context-length error instead of a paraphrase, and a call is never silently retried on the tool's behalf. `retry-after`, `x-ratelimit-*` and the provider's request id (as `x-dcdr-upstream-request-id`) are forwarded. Every gateway request now gets an `x-request-id` - the client's own when it sends one, minted otherwise - and the same id is on its execution log. The log carries the usage the provider reported, streamed or not (the gateway asks for it on a stream and keeps that chunk to itself unless the client asked too), so budgets can account gateway spend. `/v1/responses` is refused with `route_not_supported_for_provider` for a provider whose wire has no Responses API. New environment: `DCDR_GATEWAY_UPSTREAM_TIMEOUT_MS` (status line, default 240 s) and `DCDR_GATEWAY_UPSTREAM_IDLE_TIMEOUT_MS` (silence mid-stream, default 120 s).
- <kbd>RUNTIME</kbd> **The gateway speaks the Anthropic Messages wire, and Claude and Gemini join its upstreams.** `POST /v1/messages` (and `/v1/messages/count_tokens`) forward an Anthropic Messages request to the tenant's bound `ANTHROPIC` credential as sent - `system`, tool-use blocks, `cache_control`, `anthropic-version` and `anthropic-beta` included - and relay the answer as received, streamed or not, so Claude Code and the Anthropic SDKs work against the gateway with a base URL and a DCDR service token in `x-api-key` or as a bearer. Gateway errors on that route use Anthropic's error envelope. A bound `ANTHROPIC` or `GEMINI` credential now also serves `/v1/chat/completions` through each vendor's OpenAI-compatible endpoint and its models appear on `/v1/models`; `/v1/responses` is refused for providers whose wire has none. Usage on the log includes cached prompt tokens from both wires.
- <kbd>RUNTIME</kbd> **`POST /v1/embeddings` on the gateway.** Forwarded as sent to `OPEN_AI`, `MISTRAL`, `GEMINI` (through its OpenAI-compatible endpoint) or `OPEN_AI_COMPATIBLE`, relayed as received, never streamed. The `model` must be an embeddings model of the bound provider - an embeddings model cannot answer a chat route and a chat model cannot answer this one - and embeddings models now appear on `/v1/models` for providers that serve the route. The log records how many inputs the request carried and the usage the provider reported.
- <kbd>CONTRACTS</kbd> **Embeddings models are priced.** OpenAI's three, Mistral's two and `gemini-embedding-2` carry their vendor's per-million input rate (snapshot 2026-09-14), with an output rate of zero because embeddings bill no output - so an embeddings call through the gateway no longer costs nothing in the billing snapshot.
- <kbd>CONTRACTS</kbd> **`@dcdr/contracts` 3.14.0 opens the gateway parity wave.** `GatewayLogSurface.MESSAGES` and `EMBEDDINGS`, `GatewayLogResponseSummary.firstChunkMs`, `GatewayLogRequestSummary.inputCount`, and `cachedPromptTokens` on execution usage. Additive.
### Fixed
- <kbd>RUNTIME</kbd> **Provider Limits no longer charge a call for every candidate an intent could have used.** The calls window was consumed while planning, once per candidate, so an intent with three implementations of the same provider used three units per request and a candidate that was never tried still cost one. A unit is now taken when a call is actually sent, once per attempt, and a response served from cache takes none.
- <kbd>RUNTIME</kbd> **Streamed Anthropic, Gemini and xAI intent calls report their token usage.** Streaming calls to these providers returned no usage at all, so they reached the logs and billing as free; the usage the provider streams is now collected (and requested, for xAI) and reported like a non-streamed call.
- <kbd>RUNTIME</kbd> **Anthropic intent calls count their cached prompt.** Usage reported only the uncached part of the input, so a call that read most of its prompt from cache looked almost free.
Security hardening of the runtime (the externally reachable process) and the workflow runner. Entries are deliberately high-level; the internal write-ups are `internal_docs/SECURITY_HARDENING_NOTES_2026-09-13.md` and `SECURITY_HARDENING_NOTES_2026-09-14.md` (the `/v1` gateway surface), and coverage is tracked in `internal_docs/SECURITY_TEST_MATRIX.md`.
- <kbd>RUNTIME</kbd> **Hardened long gateway calls and service-token limits.** A non-streamed gateway call that takes longer than the runtime's general request timeout is no longer answered early while the provider call carries on; counters given back after a refused call can no longer outlive their window, and limit rules a runtime cannot read are ignored and reported instead of guessed at.
- <kbd>RUNTIME</kbd> **Hardened the `/v1` gateway surface after the 3.14.0 wave.** The gateway token is read from headers only; a client that stops reading a stream is disconnected after `DCDR_GATEWAY_CLIENT_DRAIN_TIMEOUT_MS` (default 60 s) and its upstream call aborted; the few client headers that travel to the upstream, and the fields the gateway copies from client or upstream bodies into its logs, are bounded in shape and length.
...
<!--
sourceCommit: be4dfa60ecea84e06859dbf788fbeb874bfc8adb
queuedAtUtc: 
previousMirroredBuild: 20260912.2 (2026-09-12)
contractsSubmodule: 952d9d3e2a8f..4c89ea4ff8eb
-->

### Added
- <kbd>RUNTIME</kbd> **A Docker E2E that proves what actually leaves the process** (`tests/e2e/agent/governance.e2e.test.ts`, `tests/e2e/agent/provider/server.cjs`, `DCDR_E2E_AGENT=1 npx jest -c jest.e2e.agent.config.cjs governance`). A provider stand-in container speaks enough of the OpenAI wire format for the runtime's SDK and records **every request body it receives**, which is the only way to answer the question a governance review actually asks: a redaction that happens in a function and not on the wire is not a redaction. One customer record - an email, a national id and an `sk-` token, each chosen to match the engine's own detectors - is planted in a real run against the shipped images and follows two exits: an `INTENT` state through the runtime, and a `TOOL` state performed by the runner. It calls no real provider and costs nothing, deliberately: a control you only test when you are willing to spend money is a control you will stop testing.
- <kbd>RUNTIME</kbd> **`ThinkingLevel` is verified against the live provider APIs, not only mocked SDKs** (`tests/e2e/providers/thinking.levels.e2e.test.ts`, 11/11, behind `DCDR_E2E_THINKING_ENABLED=true` because reasoning tokens cost real money). The enum was defined from a type union, and a type union is a promise about an API rather than the API: `gpt-5-mini` was asked for every rung and accepted all seven, **including `xhigh` and `max`**, so no DCDR level is one the provider has to fold into another. The suite also proves the sampling reconciliation is load-bearing rather than superstition by asking Anthropic for the combination directly and recording what comes back — `400 invalid_request_error: "temperature may only be set to 1 when thinking is enabled"` — and then making the same call through the adapter, which succeeds because the adapter dropped it.
- <kbd>RUNTIME</kbd> **Anthropic honours extended thinking, which it had never implemented.** `enable_thinking` was never read in `anthropic.provider.ts`: a caller asking Claude to think got none, silently, and the request went out as though they had not asked. The adapter now builds a real `thinking` block from the requested `ThinkingLevel`, with the budget resolved through the contract's shared table and clamped against the request's own `max_tokens` — Anthropic draws reasoning tokens from that same allowance and refuses a budget that does not leave room for an answer. An allowance too small to fund thinking above the provider's 1 024 floor sends no thinking block and says so in a warning, rather than sending a value the provider rejects.
- <kbd>RUNTIME</kbd> **Sampling and reasoning are reconciled in the adapter, not in every caller.** Asking for thinking now drops `temperature` and `top_p` where the provider constrains them: Anthropic pins temperature while extended thinking runs and refuses `top_p` with it, and on OpenAI the existing GPT-5 rule is extended to any request that actually carries a `reasoning_effort` — the o-series was previously uncovered, so a caller asking for thinking on `o3` was still sending a temperature. This is the ask the backend raised: a caller should be able to say "think hard" and "be deterministic" and have the adapter resolve the conflict for the provider it is talking to. Today every caller had to know three providers' rules, which is how most of them get it wrong quietly, in the direction of non-determinism — and the backend had pinned `temperature: 1` on the managed planner to cope with a constraint that, on Anthropic, bought nothing at all because thinking was a no-op there.
- <kbd>RUNTIME</kbd> **Gemini sends the level's budget instead of `thinkingBudget: -1`.** `-1` means "the model decides", which made every level identical and unpredictable to price. A unit test that previously asserted `-1` was requesting an unbounded reasoning budget against `max_tokens: 16`; it now pins the three behaviours that matter — the level's published budget, the clamp when the allowance is tight, and thinking off rather than unbounded when the allowance cannot fund it at all.
- <kbd>CONTRACTS</kbd> **Submodule bumped to `@dcdr/contracts` 3.11.0 — `PromptParameters.max_tokens` accepts up to 200 000** (was 8 192), so a prompt can be configured for the output window the model actually has. Note for anyone mirroring the rule: `equivalo-backend` validates the same field with its own `z.number().max(8192)` in the intent draft path, and `workflow.managed.intents.seed.ts` pins the platform agent intents at 8 192 while calling it "the contract's ceiling" — both need raising there or a value this contract now accepts is still refused on the way in. The version is 3.11.0 because **3.10.0 is published** and is `latest` on npm — a released artifact the backend is running on, not a free number. (An earlier draft of this entry said it had never been published; that reading was taken before the owner published it at midday and the backend session corrected it in the handoff the same day.)
- <kbd>RUNTIME</kbd> **A signed registry now unlocks pro capabilities in a self-hosted runtime, which is what it was always supposed to do.** The platform's answer to "how does a self-hosted deployment get cloud/pro features" is a signature: the backend signs the registry `sha256` with a trusted Ed25519 key and the runtime verifies it. `sync.service.ts` honoured that; `registry.file.service.ts` — the only loader a self-hosted deployment uses — parsed `signature` and `signatureKeyId`, copied them onto the registry, and then refused on capabilities **without ever consulting them**, so no signature could lift the gate and the privacy engine (PII redaction, secret detection, provider policy) could not be switched on at all. The decision now lives once, in `src/services/registry.capabilities.service.ts`, and both loaders call it; the freeware allowlist had been declared twice and is declared once. The refusal for an unsigned registry ends with the remedy the sync path always gave instead of stopping at an internal capability key. Widening a gate is where gates disappear, so the suite asserts both directions — a validly signed registry loads, and one signed by an untrusted key, signed over different bytes, or unsigned still does not — and the 13 pre-existing freeware/signature gating cases pass unchanged. **Found by the governance audit, and the fix is what made its positive test possible**: the E2E now runs the same canary workflow twice, once with no engine (all three canaries reach the model verbatim) and once on a signed registry (none of them survives to the wire). Was G0 in the audit.
- <kbd>RUNTIME</kbd> **A registry the loader accepts can fail every call with a `500`.** `IntentContract.retryPolicy` is required by the contract and unchecked by the loader, which validates "only a minimal shape" by its own comment. An intent missing it loads cleanly, logs `Local registry loaded: intents=1`, and the first execution reads `policy.allowFallback` off `undefined` and answers `INTERNAL_ERROR` with a `TypeError` — so a self-hosted operator's misconfiguration reports as a broken product. Found while building the governance fixture, pinned by `tests/services/registry.processing-capability.test.ts`, tracked as G9.
- <kbd>RUNTIME</kbd> **A source-level AI governance audit of both repositories** (`internal_docs/AI_GOVERNANCE_AUDIT.md`, tracked as `RM-057`). Nine findings, each with the file and line to re-check it, ranked by what would fail an enterprise audit rather than by effort. Every one is the same shape - a control declared in a contract and enforced nowhere - which is worse than a missing feature: a missing feature is a scope conversation, a hollow control is a misrepresentation. The two that matter: **evidence assurance is never set by the runner and defaults to `ENFORCED` in the backend**, so citations a planner model invented are stored at the level that means "the platform executed this itself and the record cannot be wrong"; and **`qcSampling` is published in the workflow contract, read by nobody in the runtime, and implemented in the backend under a different shape** with one of its four fields, so `onQcFail: PARK` produces a log line and nothing else. The document also records what holds - the closed tool catalogue, tenant-grants/author-narrows, governance gates on publish and on answering a task, deterministic QC sampling over the existing review queue, and 31 implemented processing rules at a mandatory `GLOBAL` scope - because a control matrix is only useful if the boundary is stated in both directions. No behaviour changed in this entry; remediation is `RM-057`.

## [20260912.2] — 05:10UTC

<!--
sourceCommit: 0054ff027994179b2651010a99ce200cc6782974
queuedAtUtc: 
previousMirroredBuild: 20260911.6 (2026-09-11)
contractsSubmodule: 443879f2442f..952d9d3e2a8f
-->

### Added
- <kbd>RUNTIME</kbd> **An agent E2E suite that runs the real runner against real hosts** (`tests/e2e/agent/`, `jest.e2e.agent.config.cjs`, `DCDR_E2E_AGENT=1`). Four containers: `dcdr-workflow-runner` from its own image, the runtime in `--registry` mode serving the platform planner and summarizer against OpenAI, a control plane answering the twelve protocol routes and recording everything the runner says, and hosts with `NET_ADMIN`, four HTTP services and a shell. An `AGENT` state is told to leave only 22 and 8080 reachable on N hosts and the assertion is what the hosts answer afterwards, probed from outside the network. The unit suite proves the loop's arithmetic against inputs we wrote; this proves a model can satisfy the decision contract we publish, that the shipped image does it, and that something changed on the far end. Findings recorded in `internal_docs/WORKFLOW_AGENT_E2E.md`.
- <kbd>RUNTIME</kbd> **`.dockerignore`.** Both Dockerfiles `COPY . .` after an `npm install` done inside the image, so without it a developer's `node_modules` - a Windows tree - landed on top of the Linux one, and the context transfer alone was gigabytes. CI never noticed because CI checks out clean; a local build of the runtime or the runner did. `src/contracts/dist` and `src/contracts/node_modules` are deliberately kept: `npm install ./src/contracts` does not run a `prepare` script for a local directory dependency.
### Added
- <kbd>RUNTIME</kbd> **`dcdr wf-resume` can address one parked scope: `--frame-id`.** The CLI sent the payload and nothing else, which is unanswerable for any run holding more than one open task - a `FOREACH` asking a person per item parks three, and the control plane refuses an unaddressed answer for those rather than guessing which was meant. `wf-tasks` prints the id and the readable path on every row; omitting the flag stays correct for the ordinary single park, where the server resolves the only parked frame itself. Covered by `tests/cli/dcdr.cli.workflow.test.ts` against a real HTTP server, both with and without the flag, asserting what reaches the wire.
- <kbd>CONTRACTS</kbd> **`WorkflowTask` carries `frameId` and `path`** (v3.10.0). The inbox rows the control plane serves had both from the day frames landed; the published type declared neither, so the data arrived and was invisible to TypeScript. Same review pass as the CLI flag above, which also caught two more of the same shape: `DcdrClient.resumeWorkflowRun` still declared `payload: unknown`, so the composed client said nothing about frames to anyone reading it, and the published `answer-tasks-example.ts` answered by run id alone. Every consumer surface - client, composed client, CLI, runnable examples, published docs - was checked against the frames model one by one, which is what the release rules ask for and what the first pass had skipped.
- <kbd>CONTRACTS</kbd> **The published workflow docs were rewritten for frames** (`docs/WORKFLOWS.md`, `docs/WORKFLOW_CLIENT.md`, `docs/CLI.md`). `WORKFLOWS.md` still announced itself as contracts 3.0.0, carried a duplicated closing section, and pointed readers at `runAndWait` / `runAndWaitByKey` - two methods that do not exist on the client. It now documents the execution model a run actually follows (the frame tree, several scopes parked at once, a finished branch being read rather than re-run, `correlation` as what tells two identical tasks apart), the `TOOL` state that was missing from the state table, and a limits table saying what each ceiling really bounds. `WORKFLOW_CLIENT.md`'s resume example ignored `frameId` entirely, which is the same defect as the CLI one in documentation form.
- <kbd>RUNTIME</kbd> **A run can be parked in several places at once.** Execution state is now a tree of **frames** — one row per scope (`ROOT`, `PARALLEL_BRANCH`, `FOREACH_ITEM`, `SUBWORKFLOW`, `AGENT`), joined by `parentFrameId`, each carrying its own position, its own `states` snapshots, its own `wait` and its own answer — replacing the single `currentStateId` + `waitStateId` cursor and the bespoke `WorkflowRunnerSubworkflowCursor`. That is what lifts the restriction a `WAIT` had inside a `PARALLEL` branch or a `FOREACH` body: the reason was never addressing, it was that a run could carry exactly **one** open wait, so two branches stopping at once had nowhere to be recorded. Now each stops on its frame, the output reports `waits[]` with one entry per parked frame, and a resume names the frame it answers (`resuming`), so a three-host audit puts three tasks in three inboxes and joins when the last one lands. A scope that a **descendant** blocked is stored `WAITING` with no `wait` of its own: it is re-entered to reach the frame that holds the task and never appears in anybody's inbox. The half that matters in production: a branch that had already finished is **read from its frame, not run again**, however many times its item is re-entered to reach a parked sibling — without it every requeue would repeat the side effects the composite had already caused. An `AGENT` inside a composite is still refused (`NESTED_AGENT_NOT_ALLOWED`): it parks on a cursor of its own that no child frame carries. Covered by `tests/workflow-runner/workflow-runner.composite.test.ts` and, against real containers, by `tests/e2e/agent/complex.e2e.test.ts` — a `FOREACH` of `PARALLEL`s that each stop for a person, then a root event, then a child workflow that parks on a `DELAY`.
- <kbd>RUNTIME</kbd> **`PARALLEL`, `FOREACH` and `SUBWORKFLOW` are implemented.** All three were contract-complete and had no execution behind them: declared `isHostExecutedWorkflowState`, accepted by the validator, drawn by the editor with their own node and i18n, sold as Cloud Pro in `WORKFLOW_ADVANCED_STATE_TYPES` — and rejected by the runner's dispatcher with `STATE_UNSUPPORTED: not supported by this runner yet (Phase 2)`. A tenant could publish a `FOREACH` and find out only when the run died; it is also why no example or fixture anywhere in the repository used `SUBWORKFLOW`, since nobody could have written one that ran. They share one engine, `runNestedGraph`, because all three are "run this small graph and give me its output" and three loops would be three places for the transition rules, the error policy and the loop guard to drift apart. `FOREACH` truncates at `maxItems` rather than failing, keeps results in **input order** whatever the concurrency does, records a failed item as `null` in its own slot, and counts its transitions against the **run's** guard — a per-scope counter would not see a `FOREACH` over ten thousand items, which is the runaway the ceiling exists for. A `SUBWORKFLOW` child runs **inline in the parent run**, with its own mapped input, its own `states` and its own identity, so one business operation keeps one audit trail and the control plane needs no child-run lifecycle. Nested records are checkpointed with the composite's own record under `parentStateId` / `branchId`, in one checkpoint, because the composite is the durability unit: two branches advance at once and cannot share a cursor. Covered by `tests/workflow-runner/workflow-runner.composite.test.ts` and by a deterministic, model-free E2E over every primitive (`tests/e2e/agent/primitives.e2e.test.ts`).
### Fixed
- <kbd>RUNTIME</kbd> **An `HTTP` state asks for connection credentials only when the connection declares it has some.** It asked always, ignoring `requiresCredentials`, where a `TOOL` state already asks only for a `CONNECTION`-brokered capability. An `HTTP` connection to an open endpoint has no secrets to issue, and a control plane answering `404` to that request — a legitimate answer — ended the **whole run** with `RUNNER_ERROR` instead of failing one state. Found the first time the deterministic E2E ran an `HTTP` state.
- <kbd>RUNTIME</kbd> **An `array<object>` prompt variable no longer reaches the model double-encoded.** `validateAndNormalizeContract` normalised **each item** of an array through `normalizeForInterpolation` and then stringified the array of strings, so a declared `array<object>` arrived as a JSON array of JSON *strings*. This is not a corner case: the platform agent planner's `tools` and `history` are both `array<object>`, so every turn of every agent run shipped its tool catalogue and its trace that way. Models cope, which is why it went unnoticed - the cost is tokens and legibility, paid on every call. The same code path built the synthetic item definition **without `properties`**, so an `array<object>`'s declared item shape was never enforced on input either; a missing required field passed straight through to the provider. Both covered by `tests/entities/model.semantics.array-of-object.test.ts`.
- <kbd>RUNTIME</kbd> **The agent summarizer is incremental.** It was sent everything outside the history window on **every** compaction, alongside the previous summary that already accounted for exactly those entries - so the model was asked to re-derive conclusions it had already written down, and the call grew with the square of the run length. It now receives only the entries that have just left the window, tracked by the new `WorkflowRunnerAgentCursor.summarizedEntries`. The mark advances only on success, so a failed compaction offers its entries again next turn instead of leaving a hole in the planner's memory. Measured on a 19-turn agent run against real hosts, where the later compactions ran past the runtime's request timeout and failed - and the planner, left with a six-entry window and no summary, started repeating work it had already done.
- <kbd>RUNTIME</kbd> **A run reports what it cost.** `WorkflowRunHost.callIntent` counted **one** tracked call per intent regardless of how the runtime had rated it, ignoring `report.trackedCallAccounting.trackedCallsConsumed` where the multimodal multiplier lives; and it never produced an `estimatedCost` at all, which made `WorkflowAgentStateConfig.maxEstimatedCost` a bound that could not fire, since the value it is compared against was permanently `undefined`. Both now come from the report, the money estimate through the new `ProviderModelRegistry.estimateTokenCost` reading the catalogue's token pricing. The estimate is absent rather than zero for a model the catalogue does not price, so a budget can tell "free" from "unknown". Covered by `tests/workflow-runner/workflow-runner.usage.test.ts`.
- <kbd>RUNTIME</kbd> **The request timeout is configurable (`REQUEST_TIMEOUT_MS`), and work that outlives it no longer double-writes.** The budget was hardcoded at 120 s, which fits an ordinary chat call and not a reasoning model with a generous `max_tokens` or an agent summarizer compacting a long trace; a non-positive value now disables it, for deployments that put their own budget in front. When the timeout did fire, the router then wrote the provider's answer to a response that had already been sent, throwing `Cannot set headers after they are sent to the client` - which the catch block reported as `Execution failed`, an error message about a call that had succeeded. The late result is now discarded with a warning that says so, because the provider still ran it and still charged for it. Covered by `tests/infra/app.request-timeout.integration.test.ts`.
- <kbd>CONTRACTS</kbd> **Submodule bumped to `@dcdr/contracts` 3.10.0**: `WorkflowRunnerAgentCursor.summarizedEntries` and `ProviderModelRegistry.estimateTokenCost`, both additive.
