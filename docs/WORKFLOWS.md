...
- Bounds are mandatory (`maxIterations`, capped by `WorkflowValidationCaps.maxAgentIterations`, default **25**) and optional (`maxTrackedCalls`, `maxToolErrors`, `maxDurationMs` capped by `maxAgentDurationMs` — default 30 days, never above `WORKFLOW_MAX_AGENT_DURATION_MS` = 180 days, parked time included — `maxEstimatedCost`); hitting one stops the loop with a `WorkflowAgentStopReason` (`MAX_ITERATIONS`, `BUDGET_EXHAUSTED`, `TOO_MANY_TOOL_ERRORS`, `TIMEOUT`, `CANCELED`, `PLANNER_ERROR`) and fails the state unless `finishOnBound` is set.
...
| `settings.timeoutMs` | definition, capped by `caps.maxTimeoutMs` (default 30 days; never above `WORKFLOW_MAX_TIMEOUT_MS` = 180 days) | whole-run wall clock, **parked time included** |
