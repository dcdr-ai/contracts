...
/**
 * Calls the local runtime with a typed nutrition request object.
 */
async function main(): Promise<void> {
  // eslint-disable-next-line no-console
  console.log(
    `[run] intent=NUTRITION_PROTEIN_SUGGESTION baseUrl=${baseUrl} timeoutMs=${timeoutMs}`,
  );
  ...
  const request: ExecuteIntentRequest = {
    vars: {
      ...
      includeWeeklyDiet: true,
    },
  };

  const response = await client.executeIntent(
    "NUTRITION_PROTEIN_SUGGESTION",
    request,
  );
...
main().catch((e) => {
  ...
  // eslint-disable-next-line no-console
  console.error(
    `[run] failed intent=NUTRITION_PROTEIN_SUGGESTION baseUrl=${baseUrl} error=${headerMessage}`,
  );
  if (parsedBody) {
    // eslint-disable-next-line no-console
