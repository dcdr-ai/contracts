...
/**
 * Calls the local runtime with a typed banking incident classification request.
 */
async function main(): Promise<void> {
  // eslint-disable-next-line no-console
  console.log(
    `[run] intent=BANKING_INCIDENT_CLASSIFIER baseUrl=${baseUrl} timeoutMs=${timeoutMs}`,
  );
...
main().catch((e) => {
  ...
  // eslint-disable-next-line no-console
  console.error(
    `[run] failed intent=BANKING_INCIDENT_CLASSIFIER baseUrl=${baseUrl} error=${headerMessage}`,
  );
  if (parsedBody) {
    // eslint-disable-next-line no-console
